...
export default function AmcWorkPage() {
    ...
    const handleSaveWorkNotes = async () => {
        ...
        try {
            ...
            const data = (await res.json().catch(() => ({}))) as AssetApiRow & {
                message?: string;
                work_saved_at?: string | null;
            };
            if (!res.ok) {
                throw new Error(
                    typeof data.message === "string" ? data.message : "Failed to save",
                );
            }
            resetForNewScan();
        } catch (err) {
        ...
        >
            <header className="px-0.5">
                <h1 className="text-xl font-black text-gray-900 flex items-center gap-2 sm:text-2xl">
                    <Settings className="h-6 w-6 text-[#0d4f3c] shrink-0 sm:h-7 sm:w-7" aria-hidden />
                    AMC Work
                </h1>
                <p className="text-sm text-gray-500 mt-1.5 leading-relaxed sm:mt-1">
                    Scan a barcode or enter a tag / serial to load asset details and record service notes.
                </p>
            </header>
