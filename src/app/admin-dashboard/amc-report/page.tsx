...
function mapApiRow(row: WorkRecordApiRow): WorkRecord {
    ...
    return {
        ...
        workType: str(row.work_type) || "amc",
...
export default function AmcReportPage() {
    ...
    const loadRecords = useCallback(async () => {
        try {
            setLoadError("");
            setIsLoading(true);
            const res = await fetch("/api/admin/amc/work-records?work_type=amc", { cache: "no-store" });
            const data = await res.json().catch(() => ({}));

            if (!res.ok) {
                const msg = typeof (data as { message?: string }).message === "string"
                    ? (data as { message: string }).message
                    : "Failed to load AMC reports";
                throw new Error(msg);
            }

            const rows = Array.isArray(data) ? (data as WorkRecordApiRow[]) : [];
            setRecords(rows.map(mapApiRow).filter((r) => r.id > 0));
        } catch (err) {
            setRecords([]);
            setLoadError(err instanceof Error ? err.message : "Failed to load AMC reports");
        } finally {
            setIsLoading(false);
        }
    }, []);
    ...
    const runExportExcel = async (rows: AmcReportExportRow[]) => {
        ...
        setExportBusy("excel");
        try {
            await exportAmcReportToExcel(rows);
        } catch (err) {
    ...
    const runExportPdf = async (rows: AmcReportExportRow[]) => {
        ...
        setExportBusy("pdf");
        try {
            await exportAmcReportToPdf(rows);
        } catch (err) {
            ...
            <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                <div>
                    <h1 className="text-2xl font-bold text-gray-900 flex flex-wrap items-center gap-2">
                        <BarChart3 className="h-7 w-7 text-[#06124f] shrink-0" aria-hidden />
                        AMC Report
                    </h1>
                    <p className="text-sm text-gray-500 mt-1">
                        Work records from AMC service visits — saved from employee AMC Work scans.
                    </p>
                </div>
            ...
            <div className="rounded-xl border border-gray-200 bg-white shadow-sm overflow-hidden">
                <div className="flex flex-wrap items-center justify-between gap-2 p-4">
                    <button
                        ...
                        aria-controls="amc-report-filters"
                    >
                    ...
                    <table className="min-w-[1100px] w-full">
                        ...
                        <tbody className="divide-y divide-gray-100">
                            ...
                            {!isLoading && records.length === 0 && (
                                <tr>
                                    <td className="px-4 py-8 text-sm text-gray-500" colSpan={TABLE_COL_SPAN}>
                                        {loadError || "No AMC work records yet."}
                                    </td>
                                </tr>
                            )}
            ...
            {viewDetail ? (
                <div className="fixed inset-0 z-50 flex items-center justify-center px-4 py-6">
                    ...
                    <div
                        role="dialog"
                        aria-modal="true"
                        aria-labelledby="amc-record-view-title"
                    ...
                    >
                        <div className="flex shrink-0 items-center justify-between gap-4 border-b border-gray-100 px-6 py-5">
                            <h3 id="amc-record-view-title" className="text-lg font-bold tracking-tight text-[#001540]">
                                AMC work record #{viewDetail.id}
                            </h3>
                            <button
                                type="button"
                                onClick={() => setViewDetail(null)}
