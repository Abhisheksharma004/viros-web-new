...
import OfferLetterDocumentView from "@/components/admin-dashboard/OfferLetterDocumentView";
import { downloadOfferLetterPdf } from "@/lib/offerLetterPdfExport";
import { type OfferLetterStatus } from "@/lib/offerLetterConstants";
import {
    mapApiRow,
...
function StatusBadge({ status }: { status: OfferLetterStatus }) {
    return (
        <span
...
export default function OfferLetterDetailPage() {
    ...
    useEffect(() => {
        ...
        const load = async () => {
            try {
                ...
                setOfferLetter(mapApiRow((data.offer_letter ?? data) as OfferLetterApiRow));
            } catch (err) {
    ...
    const handleDelete = async () => {
        if (!offerLetter) return;
        const ok = window.confirm(`Delete offer letter "${offerLetter.offerNumber}"? This cannot be undone.`);
        if (!ok) return;

        setIsDeleting(true);
        try {
            const resp = await fetch(`/api/admin/offer-letters/${id}`, { method: "DELETE" });
            const data = await resp.json().catch(() => ({}));
            if (!resp.ok) {
                throw new Error(typeof data.message === "string" ? data.message : "Delete failed");
            }
            router.push("/admin-dashboard/offer-letter");
        } catch (err) {
            alert(err instanceof Error ? err.message : "Unable to delete offer letter.");
        } finally {
            setIsDeleting(false);
        }
    };
        ...
        <div className="space-y-6">
            <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                ...
                <div className="flex flex-wrap items-center gap-2">
                    ...
                    <Link
                        href={`/admin-dashboard/offer-letter/${id}/edit`}
            ...
            <div className="overflow-hidden rounded-md border border-gray-200 bg-gray-50 shadow-sm">
                <OfferLetterDocumentView offerLetter={offerLetter} />
            </div>
        </div>
    );
}
