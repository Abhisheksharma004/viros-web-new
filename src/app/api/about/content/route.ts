...
export async function PUT(request: Request) {
    try {
        ...
        const { title, subtitle, description, mission, vision, video_url, cta_title, cta_subtitle, cta_primary_text, cta_secondary_text } = body;
        ...
        if (rows.length === 0) {
            await pool.query(
                'INSERT INTO about_page_content (title, subtitle, description, mission, vision, video_url, cta_title, cta_subtitle, cta_primary_text, cta_secondary_text) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
                [title, subtitle, description, mission, vision, video_url, cta_title, cta_subtitle, cta_primary_text, cta_secondary_text]
            );
        } else {
            const id = rows[0].id;
